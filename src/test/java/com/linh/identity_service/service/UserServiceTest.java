...
// h2 db do not run in this code, but we need isolation and using /test.properties
// because when spring run them will using test.properties ( h2 using RAM ) to testing,
// so it can isolation database it is convenient for tester
@SpringBootTest
...
public class UserServiceTest {
    ...
    @MockBean
    UserRepository userRepository; // create emulator and  don't actually use it during testing it mean don't use database
    ...
    void getMyinfo_userNotFound_error(){
       ...
       // when(userMapper.toUserResponse(user)).thenReturn(userResponse);
        ...
        AppException thrownException =
                Assertions.assertThrows(AppException.class, () -> userService.myInfo());
